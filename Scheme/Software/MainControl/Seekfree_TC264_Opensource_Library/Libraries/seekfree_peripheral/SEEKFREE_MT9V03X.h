...
#define MT9V03X_PCLK_PIN        ERU_CH4_REQ8_P33_7  //定义像素时钟引脚 可选范围参考ERU_PIN_enum枚举 不可与场中断引脚选择为同一个通道，或者是共用中断的通道
                                                    //例如场中断选择ERU_CH3 那么像素引脚就不能选择ERU_CH7，因为3和7的中断是共用的。
...
#define MT9V03X2_COF_UART        UART_2              //配置摄像头所使用到的串口
#define MT9V03X2_COF_UART_TX     UART2_TX_P33_9
#define MT9V03X2_COF_UART_RX     UART2_RX_P33_8

#define MT9V03X2_VSYNC_PIN       ERU_CH5_REQ1_P15_8  //场中断引脚 可选范围参考ERU_PIN_enum枚举 不可与像素引脚选择为同一个通道，或者是共用中断的通道
                                                    //例如场中断选择ERU_CH3 那么像素引脚就不能选择ERU_CH7，因为3和7的中断是共用的。

#define MT9V03X2_DATA_PIN        P15_0               //定义D0数据引脚  假设D0定义为P00_0 那么D1所使用的引脚则为P00_1，依次类推
                                                    //可设置参数P00_0、P02_0、P15_0，不能设置为其他参数

#define MT9V03X2_PCLK_PIN        ERU_CH6_REQ12_P11_10 //定义像素时钟引脚 可选范围参考ERU_PIN_enum枚举 不可与场中断引脚选择为同一个通道，或者是共用中断的通道
                                                    //例如场中断选择ERU_CH3 那么像素引脚就不能选择ERU_CH7，因为3和7的中断是共用的。
#define MT9V03X2_DMA_CH          IfxDma_ChannelId_6  //定义使用的DMA通道 0-47可选  通道号越大优先级越低
...
//-----------------------------摄像头一和二除了中断函数名 引脚名 通道号 中断优先级  linked list 不一样       其他都一样------------------
uint16 estimate_distance(uint8 beacon_len);//估算距离
#endif
