...
IFX_ALIGN(256) DMA_LINK dma2_link_list;
...
uint8 eru_dma_init(IfxDma_ChannelId dma_ch, uint8 *source_addr, uint8 *destination_addr, ERU_PIN_enum eru_pin, TRIGGER_enum trigger, uint16 dma_count)
{
	...
	if(16384 < single_channel_dma_count)            //一个transaction最大16K 个transfer
	{
		while(TRUE)
		{
			single_channel_dma_count = dma_count / list_num;
    ...
    cfg.busPriority 					= IfxDma_ChannelBusPriority_medium;    //被修改了原来是high
	...
	IfxDma_Dma_initChannel(&dmaChn, &cfg);          //开中断
...
//  Sample usage:
//-------------------------------------------------------------------------------------------------------------------
uint8 eru_dma2_init(IfxDma_ChannelId dma_ch, uint8 *source_addr, uint8 *destination_addr, ERU_PIN_enum eru_pin, TRIGGER_enum trigger, uint16 dma_count)
{
    IfxDma_Dma_Channel dmaChn;
    //eru触发DMA通道号   在eru文件中设置eru的优先级，即为触发的通道
    eru_init(eru_pin, trigger);
    ...
    else
    {
        cfg.shadowControl               = IfxDma_ChannelShadow_linkedList;
        cfg.operationMode               = IfxDma_ChannelOperationMode_continuous;
        cfg.shadowAddress               = IFXCPU_GLB_ADDR_DSPR(IfxCpu_getCoreId(), (unsigned)&dma2_link_list.linked_list[1]);
    }
    ...
    cfg.hardwareRequestEnabled          = FALSE;
    cfg.channelInterruptEnabled         = TRUE;
    cfg.channelInterruptPriority        = ERU_DMA2_INT_PRIO;            //摄像头二
    cfg.channelInterruptTypeOfService   = ERU_DMA2_INT_SERVICE;
    ...
    if(1 < list_num)
    {
        ...
        {
            cfg.destinationAddress      = IFXCPU_GLB_ADDR_DSPR(IfxCpu_getCoreId(), destination_addr + single_channel_dma_count * i);
            if(i == (list_num - 1)) cfg.shadowAddress = IFXCPU_GLB_ADDR_DSPR(IfxCpu_getCoreId(), (unsigned)&dma2_link_list.linked_list[0]);
            else                    cfg.shadowAddress = IFXCPU_GLB_ADDR_DSPR(IfxCpu_getCoreId(), (unsigned)&dma2_link_list.linked_list[i+1]);
            cfg.transferCount           = single_channel_dma_count;

            IfxDma_Dma_initLinkedListEntry((void *)&dma2_link_list.linked_list[i], &cfg);
            i++;
        }
    }


    IfxDma_Dma_getSrcPointer(&dma2_link_list.channel)->B.CLRR = 1;
