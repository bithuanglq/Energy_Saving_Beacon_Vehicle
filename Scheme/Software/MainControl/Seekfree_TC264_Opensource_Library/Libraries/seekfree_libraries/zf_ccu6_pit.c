...
//  @param      pit_ch          选择通道(PIT_CH0、PIT_CH1)
//  @return     void
//  @note               目前只能用于  CCU6_0  PIT_CH0
//  Sample usage:               pit_period(CCU6_0, PIT_CH0, 50*1000);  //ccu6_0 pit_ch0 周期为 50ms
//-------------------------------------------------------------------------------------------------------------------

void pit_period(CCU6N_enum ccu6n, CCU6_CHN_enum pit_ch,uint32 time)
{
    uint64 timer_input_clk;
    uint32 timer_period;
    uint8 i;
    ...
    module = IfxCcu6_getAddress((IfxCcu6_Index)ccu6n);
    timer_input_clk = 100000000;  //100MHz
    g_Ccu6Timer.ccu6 = module;
    g_Ccu6Timer.timer = (IfxCcu6_TimerId)(pit_ch);
    //close Timer
    IfxCcu6_Timer_stop(&g_Ccu6Timer);
    ...
    // period setting                    only for ccu6_0 pit_ch0  !!!
    uint16  prescaler;
    float32 freqCC6   = 100000000;
    uint16  divFactor = freqCC6 / timer_input_clk;
    ...
    IfxCcu6_setT12PeriodValue(module, (uint16)(timer_period - 1));
    clockInput = (IfxCcu6_TimerInputClock)(prescaler & 0x7U);
    IfxCcu6_setInputClockFrequency(module, IfxCcu6_TimerId_t12, clockInput);
